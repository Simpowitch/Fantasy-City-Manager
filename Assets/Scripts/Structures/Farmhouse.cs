...
public class Farmhouse : GoodsProducer
{
    ...
    private Task GetPlantSeedTask(Citizen citizen, List<Farmland> farmlands)
    {
        ...
        if (freeFarmTile != null)
        {
            ...
            return new Task(workTaskDescription, ThoughtFileReader.GetText(citizen.UnitPersonality, workTaskThoughtHeader), plantSeed, pos, UnitAnimator.ActionAnimation.PlantSeed);
        }
        else
        {
    ...
    private Task GetHarvestTask(Citizen citizen, List<Farmland> farmlands)
    {
        ...
        if (crops.Count > 0)
        {
            ...
            Vector3 dir = pos - citizen.transform.position;
            return new Task(workTaskDescription, ThoughtFileReader.GetText(citizen.UnitPersonality, workTaskThoughtHeader), collectTimer, pos, UnitAnimator.ActionAnimation.Harvest);
        }
        else
        {
