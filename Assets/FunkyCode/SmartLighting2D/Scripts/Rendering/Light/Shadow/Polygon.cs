...
namespace Rendering.Light.Shadow {
    ...
    public class Polygon : Base {
		...
		public static void Draw(LightingBuffer2D buffer, List<Polygon2D> polygons, float lightSizeSquared, float z, Vector2 offset, Vector2 scale) {
			if (Lighting2D.commonSettings.highQualityShadows == LightingSettings.QualitySettings.ShadowQuality.Soft) {
				...
				DrawLegacy(buffer, polygons, lightSizeSquared, z, offset, scale);
			}
		}

		public static void DrawLegacy(LightingBuffer2D buffer, List<Polygon2D> polygons,  float lightSizeSquared, float z, Vector2 offset, Vector2 scale) {
			...
			for(int i = 0; i < PolygonCount; i++) {
				...
				int pointsCount = pointsList.Count;
				 ...
				 for(int x = 0; x < pointsCount; x++) {
					...
					if (buffer.lightSource.outerAngle > 0) {
