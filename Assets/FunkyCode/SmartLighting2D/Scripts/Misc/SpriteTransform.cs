...
public struct SpriteTransform {
    ...
    public SpriteTransform(VirtualSpriteRenderer spriteRenderer, Vector2 position, Vector2 scale, float rotation) {
        ...
        textureScale.x = (float)textureWidth / spriteRect.width;
        textureScale.y = (float)textureHeight / spriteRect.height;

        scale.x /= textureScale.x;
        scale.y /= textureScale.y;

        scale.x *= (float)textureWidth / (sprite.pixelsPerUnit * 2);
        scale.y *= (float)textureHeight / (sprite.pixelsPerUnit * 2);
