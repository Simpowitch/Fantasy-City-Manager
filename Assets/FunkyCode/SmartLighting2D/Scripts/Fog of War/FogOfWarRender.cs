...
public class FogOfWarRender {
    ...
    public static void OnRender(FogOfWarBuffer2D buffer) {
        ...
        FogOfWarOnRenderMode onRenderMode = FogOfWarOnRenderMode.Get(camera, buffer);
    ...
    public static void PostRender(FogOfWarBuffer2D buffer) {
        ...
        Rendering.Universal.WithoutAtlas.Texture.Draw(buffer.GetMaterial(), LightingPosition.GetCamera(camera), LightingRender2D.GetSize(camera), camera.transform.eulerAngles.z, LightingPosition.GetCamera(camera).z);
    }
    ...
    static public void PreRender(FogOfWarBuffer2D buffer) {
        ...
        Graphics.DrawMesh(LightingRender2D.GetMesh(), Matrix4x4.TRS(LightingPosition.GetCamera(camera), camera.transform.rotation, LightingRender2D.GetSize(camera)), buffer.GetMaterial(), 0, camera);
    }
}
